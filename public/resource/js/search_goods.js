$(function(){
	...
	// 菜单隐藏和显示
	$(".side_category .sort dt").live("click",function(event){
		$(this).parents(".sort").find("dd").toggle();
	...
	/*$('.list_pic').find('dl').live('mouseover',function(){
		if($(this).find('.slide-show').html() == null){
			...
			$.getJSON('index.php?act=search&op=ajax_goods_more_image&sku_id='+id,function(data){
				...
				if(data){
					...
					param += '</ul></dd>';
					obj.after(param);
					$.getScript(MISC_URL+"/resource/js/search_goods.js");
				}else{
