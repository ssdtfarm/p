define(function(require, exports, module) {
	...
	var apiData  = {
		// 表单提交
		formAction: cdnConfig.my + "/security/editbindphonetwo",
		// 检验手机是否已经被绑定
		...
		// 成功页面
		success: cdnConfig.my + "/security/editbindphonethree"
	};
    ...
    function countDownFn(t, obj1, obj2) {
	    var tempT = t;
	    obj2.show();
	    getPhoneCodeTimer =  setInterval(function(){
		    if(t<=0){
    ...
    function chkPhoneError(flag) {
    	...
    	if(canCountDown == false) {
        	...
        	$("#J_getModifyPhoneCode").html('重新获取验证码');
        }
    	canCountDown = true;
    }
	...
	function checkPhoneIsExists(obj, arrItem) {
		...
		$.ajax({
			...
			error: function() {
				chkPhoneError();
				showTips(obj, arrItem["modifyFailedText"], "no");
			}
		});
	}
