function drop_cart_item(cart_id){
    $.getJSON('index.php?act=cart&op=nologin_drop&cart_id=' + cart_id, function(result){
        if(result.done){
            //删除成功
...
function change_quantity( rec_id, input, orig){
    ...
    var _v = input.value;
    $.getJSON('index.php?act=cart&op=nologin_update&cart_id=' + rec_id + '&quantity=' + _v, function(result){
        if(result.done){
            //更新成功
