define(function(require, exports, module) {
    ...
    var interactArr = {
        ...
        "set": [{
            ...
            type: "set",
            nullText: "请设置6位数字支付密码",
            equalLoginPswText: "支付密码不能与登录密码相同",
            errorText: "请设置6位数字支付密码",
            minLenText: "密码太简单了，为了安全，建议设置6位数字支付密码！",
            maxLenText: "密码太长了，你确定能记住吗？请设置6位数字支付密码",
            normalText: "请设置6位数字支付密码"
        }, {
            // 设置支付密码-确认支付密码
    ...
    function bindEvent() {
        ...
        if (type) {
            switch (type.toString()) {
                case "1":
                    // 设置支付密码默认提示内容
                    showTips($("#J_modifyPayPsw"), '请设置6位数字支付密码', 'normal');
                    setPswBindEvent();
                    break;
                case "2":
                    // 修改支付密码默认提示内容
    ...
    function checkIsEqual(type, arrItem) {
        ...
        if (conPassVal !== "") {
            if (newPassVal !== conPassVal) {
                ...
                if (isCheckedSetPsw) {
                    if (type === "2") {
                        showTips(newPass, "请设置6位数字支付密码", "ok");
                    } else {
                        showTips(newPass, "请设置6位数字支付密码", "ok");
                    }
                    isCheckedSetPsw = true;
