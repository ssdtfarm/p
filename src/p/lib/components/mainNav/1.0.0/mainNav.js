define(function(require, exports, module){
	...
	function mainNav(config){
		...
		//初始化函数
		function init(){
			mainCell.find("a").each(function(index){
				var ind = index;
		...
		function setDefaultLine(ind) {
			var curWidth = mainCell.find("a").eq(ind).outerWidth(true);
			var disWidth = curWidth - mainCell.find("a").eq(ind).width();
			//设置线宽以适应不同的宽度
			setLineWidth(ind);

			var left = 0;
			for(var i=0; i<ind; i++){
				left += mainCell.find("a").eq(i).innerWidth();
			}
		...
		function setLineWidth(ind){
			lineCell.css({
				"width" : mainCell.find("a").eq(ind).width()
			})
		}
	}
