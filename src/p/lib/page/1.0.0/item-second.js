define(function(require, exports, module){
    ...
    var mainNav = require('../../components/mainNav/1.0.0/mainNav');
    ...
    var minBar = require('../../components/minBar/1.0.0/minBar');
    ...
    randerQrcode(86, 86, $("#down_url").val(), $('.down-code'));
    ...
    function setServiceTemplate(data) {
        ...
        $("#J_service").html(template("item/tplItemService",data));
        //});
    }
    /**
     *  获取库存函数
                ...
                if(result.status=="succ") {
                    $.ajax({
                        ...
                        success : function(res) {
                            ...
                            // console.log(sellArr,sellArr[0],typeof sellArr[1]);
                    ...
                    sellArr.push(res.data.sell);
                    // console.log(typeof res.data.sell);
    ...
    function setButton(num) {
        if(num >1 ){
            //如果没有售空/下架，能加入购物车
            $(".item-btn-cart").attr("id","J_addToCart");
            $(".item-btn-cart").removeClass("disable");
        }else{
            //不能加入购物车
            $(".item-btn-cart").removeAttr("id");
            $(".item-btn-cart").addClass("disable");
            //立即购不能用
            $("#J_buyNow").remove();
        }
    }
        ...
        $.ajax({
            ...
            success : function(res){
                ...
                if(res.status == "succ"){
                    ...
                    $("#J_itemLeft").append(template("item/tplItemHot",rankData));
                    //});
                    loadHistory({
                        "hasMt" : true
                    });
        ...
        $.ajax({
            ...
            success : function(res){
                if(res.status=="succ"){
                    ...
                    //加载相关的组件并渲染模板
                    //seajs.use(['lib/v1/1.0.0/item/tplItemHistory'],function(tplHistory){
                    $("#J_itemLeft").append(template("item/tplItemHistory",viewedData));
                    //});
                }else{
    ...
    function randerAll(tarObj) {
        //seajs.use(['lib/v1/1.0.0/slider', 'lib/v1/1.0.0/item/tplItemRecomment'], function(slide, tplRecomment){
        // console.log(recData);
        //渲染dom
        tarObj.prepend(template("item/tplItemRecomment",recData));
        //调用tab切换
        itemTab({
            titCell : '#J_hotTitle',
        ...
        //调用交互函数
        matchInteration("hotItem");
        //});
    }
    //人气搭配 交互
    function matchInteration(domNAME){
        var obj = $("input[name="+ domNAME +"]");
    ...
    $("#J_bidHistoryBtn").on("click", function(){
        if(!bidFlag){
            ...
            itemPage({
                ...
                pageDom  : $("#J_bidPage"),
                template : template,
                dataTplName : "item/tplItemBid",
                pageTplName : "item/tplItemPage"
            });
            bidFlag = true;
        }
    });
    ...
    $("#J_commentsBtn").on("click", function(){
        if(!commentsFlag){
            ...
            itemPage({
                ...
                template : template,
                dataTplName : "item/tplItemComments",
                pageTplName : "item/tplItemPage"
            });
            commentsFlag = true;
        }
     ...
     *
     * */
    //if(window.location.href.indexOf("kinhom")>-1 || window.location.href.indexOf("jjcdn")>-1) {
    minBar({
        mainCell   : '#J_minBar',
