    ...
    function() {
        $(this).mouseover(
            function() {
                ...
                if (menu_height <45) menu.height(60);
                menu_height = menu.height();
                li_top = $(this).position().top;
                li_left = $(this).position().left;
                if ((li_top > 118) && (menu_height >= li_top)) $(menu).css("top",-li_top+1);
                if ((li_top > 230) && (menu_height >= li_top)) $(menu).css("top",-li_top+79);
                if ((li_top > 340) && (menu_height >= li_top)) $(menu).css("top",-li_top+79);
                if ((li_top > 345) && (menu_height <= li_top)) $(menu).css("top",-li_top+79);
             /* 
...
var Speed = 1; //速度(毫秒)
var Space = 6; //每次移动(px)
var PageWidth = 525; //翻页宽度
var FillWidth = 875; //同类推荐翻页宽度
var fill = 0; //整体移位
var MoveLock = false;
var MoveTimeObj;
var Comp = 0;
...
AutoPlay();
$(document).ready(function GetObj(objName){if(document.getElementById){return eval('document.getElementById("'+objName+'")')}else{return eval('document.all.'+objName)}}
...
GetObj('ISL_Cont_875').scrollLeft = fill;
GetObj("ISL_Cont_875").onmouseover = function(){clearInterval(AutoPlayObj);}
GetObj("ISL_Cont_875").onmouseout = function(){AutoPlay();}
AutoPlay();
function GetObj(objName){if(document.getElementById){return eval('document.getElementById("'+objName+'")')}else{return eval('document.all.'+objName)}}
function ISL_GoUp_875(){ //上翻开始
 if(MoveLock) return;
 clearInterval(AutoPlayObj);
 MoveLock = true;
 MoveTimeObj = setInterval('ISL_ScrUp_875();',Speed);
}
function ISL_StopUp_875(){ //上翻停止
 clearInterval(MoveTimeObj);
 if(GetObj('ISL_Cont_875').scrollLeft % FillWidth - fill != 0){
  Comp = fill - (GetObj('ISL_Cont_875').scrollLeft % FillWidth);
  CompScr_875();
 }else{
  MoveLock = false;
 }
...
function ISL_ScrUp_875(){ //上翻动作
 if(GetObj('ISL_Cont_875').scrollLeft < 0){GetObj('ISL_Cont_875').scrollLeft = GetObj('ISL_Cont_875').scrollLeft + GetObj('List3').offsetWidth}
 GetObj('ISL_Cont_875').scrollLeft -= Space ;
}
function ISL_GoDown_875(){ //下翻
 clearInterval(MoveTimeObj);
 if(MoveLock) return;
 clearInterval(AutoPlayObj);
 MoveLock = true;
 ISL_ScrDown();
 MoveTimeObj = setInterval('ISL_ScrDown_875()',Speed);
}
function ISL_StopDown_875(){ //下翻停止
 clearInterval(MoveTimeObj);
 if(GetObj('ISL_Cont_875').scrollLeft % FillWidth - fill != 0 ){
  Comp = FillWidth - GetObj('ISL_Cont_875').scrollLeft % FillWidth + fill;
  CompScr_875();
 }else{
  MoveLock = false;
 }
...
function ISL_ScrDown_875(){ //下翻动作
 if(GetObj('ISL_Cont_875').scrollLeft >= GetObj('List3').scrollWidth){GetObj('ISL_Cont').scrollLeft = GetObj('ISL_Cont_875').scrollLeft - GetObj('List3').scrollWidth;}
 GetObj('ISL_Cont_875').scrollLeft += Space ;
}
function CompScr_875(){
 var num;
 if(Comp == 0){MoveLock = false;return;}
 if(Comp < 0){ //上翻
  if(Comp < -Space){
  ...
  GetObj('ISL_Cont_875').scrollLeft -= num;
  setTimeout('CompScr_875()',Speed);
 }else{ //下翻
  if(Comp > Space){
   Comp -= Space;
   num = Space;
