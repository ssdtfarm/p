...
function initSlider(config){
	...
	var warpObj = config['dom'] || "sliderList_0"; //包裹层domid
	...
	var aPosition = [
		...
		{width:340,height:246,top:124,left:0,zIndex:6},   //第三张	
		{width:340,height:246,top:124,left:850,zIndex:6}, //第四张
		{width:540,height:390,top:55,left:550,zIndex:8},  //第五张
	]
	...
	//for(i=0;i<oTli.length;i++){
		...
		myAddEvent(oTli[i], 'mouseout', function(){
			if(this.className != 'hove')startMove(this, {opacity:20});
		})
	...
	for(i=0;i<aLi.length;i++){
		...
		//默认载入时设置当前有标题,如果参数showTitle设置为true时才会操作标题
		//没有文字内容时不显示内容框
		//console.log(aP[i].innerHTML);
		if(showTitle&&(!(aP[iNow].innerHTML===""))) {
			if(aLi[i].style.width == '690px'){
				startMove(aP[i], {bottom:0});
			}
		}
	...
	function sMove(){
		for(i=0;i<aLi.length;i++){
			...
			aLi[i].className = '';
			//判断索引是否是当前元素，设置对应标题,如果参数showTitle设置为true时才会操作标题
			//没有文字内容时不显示内容框
			if(showTitle) {
				if((i==iNow)&&(!(aP[iNow].innerHTML===""))){
					startMove(aP[i], {bottom:0});
				}else{
