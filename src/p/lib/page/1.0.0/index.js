define(function(require, exports, module) {
    ...
    var templateIndex = require('../../template/tempIndex');
    ...
    $(".JQ_preload").each(function() {
        ...
        that.html(templateIndex(tplName)).removeClass("lazy-preview");
    ...
    khIndex.fullSlider = function(ele, tplName, data) {
        try {
            ...
            ele.html(templateIndex(tplName, tempData));
    ...
    khIndex.recomSlider = function(ele, tplName, data) {
        try {
            var len = data.length;
            ele.html(templateIndex(tplName, data));
    ...
    khIndex.special = function(ele, tplName, data) {
        ...
        try {
            ...
            if (dIndex > -1) {
                //填充数据
                ele.html(templateIndex(tplName, data[dIndex]));
                //调用切换
        ...
        try {
            ...
            if (len >= 4) {
                ...
                if (data.length == 0) {
                    ...
                    ele.html(templateIndex(tplName, tData));
    ...
    khIndex.wrapAd = function(ele, tplName, data) {
        ...
        try {
            ...
            if (dataIndex > -1) {
                ele.html(templateIndex(tplName, data[dataIndex]));
            } else {
                ele.remove();
            }
        } catch (e) {
    ...
    try {
        if (navigator.userAgent.indexOf("MSIE 8.0") > -1 || navigator.userAgent.indexOf("MSIE 7.0") > -1) {
            ...
            var normalWidth = 1430;

            $(window).resize(function() {
                winWidth = $(window).width();
                if (winWidth < normalWidth) {
                    $(".float-custom").hide();
                } else {
                    $(".float-custom").show();
                }
