...
function ISL_GoUp(){ //上翻开始
 if(GetObj('ISL_Cont').scrollLeft == 0){MoveLock = true;}
 if(MoveLock) return;
...
function ISL_StopUp_875(){ //上翻停止
 clearInterval(MoveTimeObj);
 if(GetObj('ISL_Cont_875').scrollLeft % PageWidth - fill != 0){
  Comp = fill - (GetObj('ISL_Cont_875').scrollLeft % PageWidth);
  CompScr_875();
 }else{
  MoveLock = false;
 }
...
function ISL_StopDown_875(){ //下翻停止
 clearInterval(MoveTimeObj);
 if(GetObj('ISL_Cont_875').scrollLeft % PageWidth - fill != 0 ){
  Comp = PageWidth - GetObj('ISL_Cont_875').scrollLeft % PageWidth + fill;
  CompScr_875();
 }else{
  MoveLock = false;
 }
